...
- What it does: Polls configured RSS feeds (e.g., BBC, The Hindu) and posts the latest items (title, short summary, link) to a Telegram group when requested.

Key Features
- Fetches RSS feeds and parses entries
- Commands to request news (e.g., `/dailynews` for BBC, `/localnews` for The Hindu)
...
- Python
- Libraries used (from repo): requests, feedparser (and likely a Telegram bot package or simple HTTP requests to the Bot API)
...
3. Install dependencies:
   ...
   # or use a requirements.txt if provided: pip install -r requirements.txt
...
5. Run the bot:
   python bot.py
   # or run the main script name used in repo (check README or the file names)
...
- Basics of polling vs webhook approaches (discussion point for interview)

Future improvements
- Add error handling and retry logic for network failures
- Use webhooks (or a hosted runner) for more reliable delivery instead of continuous polling
- Add simple logging and unit tests for parsing logic
- Add configuration for multiple feeds and per-feed scheduling

Notes for interview
- Explain how you parse an RSS item and format it for Telegram.
- If asked about scaling, say that for a student project you focused on correctness; production needs would include webhooks, hosted services, and rate limiting.